"""Single-file implementation for Trust Region Policy Optimization (TRPO) in TFBind-8 environment.

Run the script with the following command:
```bash
python baselines/TRPO_tfbind.py
```
...
def train_step(idx: int, train_state: TrainState) -> TrainState:
    ...
    def extract_advantage_components(
        ...
        current_env: gfnx.TFBind8Environment,
        current_env_params: gfnx.TFBind8EnvParams,
    ):
...
@hydra.main(config_path="configs/", config_name="TRPO_tfbind", version_base=None)
def run_experiment(cfg: OmegaConf) -> None:
    # Log the configuration
    log.info(OmegaConf.to_yaml(cfg))
    if cfg.agent.train_backward:
        raise ValueError("TRPO_tfbind keeps P_B fixed; set agent.train_backward=false.")
