...
class GFNAlgorithm:
    ...
    def get_random_action_prob(self, it: int):
        ...
        if self.global_cfg.algo.train_det_after is not None and it >= self.global_cfg.algo.train_det_after:
            return 0
        prob = self.global_cfg.algo.train_random_action_prob
        anneal_steps = self.global_cfg.algo.train_random_action_prob_anneal_steps
        if anneal_steps is not None:
            if anneal_steps <= 0:
                return 0
            prob *= max(0.0, 1.0 - it / anneal_steps)
        return prob
