...
class BinarySEHDataset(Dataset):
    ...
    def setup(self, task: SEHTask, ctx: FragMolBuildingEnvContext) -> None:
        with self.path.open("rb") as handle:
            binary_mols = pickle.load(handle)
        graphs: List[Graph] = []
        rdmols: List[RDMol] = []
        skipped: List[int] = []
        for idx, binary_mol in enumerate(binary_mols):
            try:
...
class SEHFragTrainer(StandardOnlineTrainer):
    ...
    def setup_data(self):
        ...
        large_test_path = self.cfg.task.seh.large_test_mols_path
        self.large_test_data = BinarySEHDataset(large_test_path) if large_test_path else None
    ...
    def setup(self):
        ...
        validation_dataset_specs = [("25", self.test_data)]
        if self.large_test_data is not None:
            self.large_test_data.setup(self.task, self.ctx)
            validation_dataset_specs.append((str(len(self.large_test_data)), self.large_test_data))
        self.validation_dataset_specs = validation_dataset_specs
