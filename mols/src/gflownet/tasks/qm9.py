...
from gflownet.trainer import StandardOnlineTrainer
...
class QM9GapTask(GFNTask):
    ...
    def __init__(
        ...
        self.debug_timing = bool(_cfg_value(cfg, "debug_timing", False)) or os.environ.get("GFN_DEBUG_TIMING", "0") == "1"
        self.slow_stage_seconds = float(_cfg_value(cfg, "slow_stage_seconds", 60.0))
        self.rdkit_conformer_timeout_seconds = int(
            _cfg_value(cfg.task.qm9, "rdkit_conformer_timeout_seconds", 0)
        )
    ...
    def compute_obj_properties(self, mols: List[RDMol]) -> Tuple[ObjectProperties, Tensor]:
        ...
        for idx, mol in enumerate(mols):
            if self.debug_timing:
                print(
                    f"[timing][qm9] mol2graph:start idx={idx} "
                    f"timeout={self.rdkit_conformer_timeout_seconds}s",
                    flush=True,
                )
            start = time.perf_counter()
            graph = mxmnet.mol2graph(  # type: ignore[attr-defined]
                mol,
                timeout_seconds=self.rdkit_conformer_timeout_seconds,
            )
            elapsed = time.perf_counter() - start
            if self.debug_timing or elapsed >= self.slow_stage_seconds:
                prefix = "[timing][qm9]" if self.debug_timing else "[slow][qm9]"
                print(
                    f"{prefix} mol2graph:done idx={idx} elapsed={elapsed:.3f}s valid={graph is not None}",
                    flush=True,
                )
            graphs.append(graph)
