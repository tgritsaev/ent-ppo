...
class DataSource(IterableDataset):
    def __init__(
        ...
        start_at_step: int = 0,
        allow_uneven_iterators: bool = False,
    ):
        ...
        self.allow_uneven_iterators = allow_uneven_iterators
    ...
    def _timing_enabled(self):
        value = getattr(self.cfg, "debug_timing", False)
        return bool(False if value == MISSING else value) or os.environ.get("GFN_DEBUG_TIMING", "0") == "1"

    def _slow_stage_seconds(self):
        value = getattr(self.cfg, "slow_stage_seconds", 60.0)
        return float(60.0 if value == MISSING else value)

    def _timing(self, message):
        if self._timing_enabled():
            print(f"[timing][data_source][eval={self.is_algo_eval}][iter={self.current_iter}] {message}", flush=True)

    def _timing_done(self, stage: str, elapsed: float, details: str = ""):
        if self._timing_enabled():
            suffix = f" {details}" if details else ""
            self._timing(f"{stage}:done elapsed={elapsed:.3f}s{suffix}")
        elif elapsed >= self._slow_stage_seconds():
            suffix = f" {details}" if details else ""
            print(
                f"[slow][data_source][eval={self.is_algo_eval}][iter={self.current_iter}] "
                f"{stage} elapsed={elapsed:.3f}s{suffix}",
                flush=True,
            )
    ...
    def __iter__(self):
        ...
        while True:
            ...
            self._timing("next_iterator_outputs:start")
            iterator_outputs = [next(i, None) for i in its]
            self._timing_done("next_iterator_outputs", time.perf_counter() - start_time)
            if any(i is None for i in iterator_outputs):
                if not all(i is None for i in iterator_outputs):
                    if not self.allow_uneven_iterators:
            ...
            for d in batch_infos:
                batch_info.update(d)
            start_time = time.perf_counter()
            self._timing(f"create_batch:start num_trajs={len(trajs)}")
            batch = self.create_batch(trajs, batch_info)
            self._timing_done("create_batch", time.perf_counter() - start_time, f"num_trajs={len(trajs)}")
            yield batch
    ...
    def do_sample_model(self, model, num_from_policy, num_new_replay_samples=None):
        ...
        def iterator():
            while self.active:
                ...
                start_time = time.perf_counter()
                self._timing(f"sample_model:start num_samples={num_samples}")
                trajs = self.algo.create_training_data_from_own_samples(model, num_samples, cond_info["encoding"], p)
                self._timing_done("sample_model", time.perf_counter() - start_time, f"num_samples={num_samples}")
                start_time = time.perf_counter()
                ...
                self._timing_done("properties_rewards", time.perf_counter() - start_time, f"num_trajs={len(trajs)}")
                self.send_to_replay(trajs[:num_new_replay_samples])  # This is a no-op if there is no replay buffer
                for traj in trajs:
                    traj["is_elbo"] = True
                    traj["is_proxy_eubo"] = False
    ...
    def do_sample_model_n_times(self, model, num_samples_per_batch, num_total):
        ...
        def iterator():
            while self.active:
                ...
                # TODO: in the cond info refactor, pass the whole thing instead of just the encoding
                start_time = time.perf_counter()
                self._timing(f"sample_model_n_times:start num_samples={n_this_time}")
                trajs = self.algo.create_training_data_from_own_samples(model, n_this_time, cond_info["encoding"], p)
                self._timing_done("sample_model_n_times", time.perf_counter() - start_time, f"num_samples={n_this_time}")
                start_time = time.perf_counter()
                self.set_traj_cond_info(trajs, cond_info)  # Attach the cond info to the trajs
                self.compute_properties(trajs, mark_as_online=True)
                self.compute_log_rewards(trajs)
    ...
    def do_dataset_in_order(
        self,
        data,
        num_samples,
        backwards_model,
        proxy_eubo_label: str | None = None,
        ...
        def iterator():
            ...
            for idcs in self.iterate_indices(total, num_samples):
                ...
                self._timing_done("dataset_backward", time.perf_counter() - start_time, f"num_samples={len(idcs)}")
                start_time = time.perf_counter()
                self.set_traj_cond_info(trajs, cond_info)  # Attach the cond info to the trajs
                self.set_traj_props(trajs, props)
                self.compute_log_rewards(trajs)
                self._timing_done("dataset_rewards", time.perf_counter() - start_time, f"num_trajs={len(trajs)}")
                is_proxy_eubo = self.is_algo_eval and getattr(getattr(self.algo, "cfg", None), "do_sample_p_b", False)
                for traj in trajs:
                    traj["is_elbo"] = False
                    traj["is_proxy_eubo"] = is_proxy_eubo
    ...
    def create_batch(self, trajs, batch_info):
        ...
        batch.elbo_mask = torch.tensor([t.get("is_elbo", False) for t in trajs], dtype=torch.bool)
        batch.proxy_eubo_mask = torch.tensor([t.get("is_proxy_eubo", False) for t in trajs], dtype=torch.bool)
        proxy_eubo_labels = sorted(
            {
                t.get("proxy_eubo_label")
                for t in trajs
                if t.get("is_proxy_eubo", False) and t.get("proxy_eubo_label") is not None
            }
        )
        for label in proxy_eubo_labels:
            safe_label = "".join(ch if ch.isalnum() else "_" for ch in str(label))
            setattr(
                batch,
                f"proxy_eubo_{safe_label}_mask",
                torch.tensor(
                    [
                        t.get("is_proxy_eubo", False) and t.get("proxy_eubo_label") == label
                        for t in trajs
                    ],
                    dtype=torch.bool,
                ),
