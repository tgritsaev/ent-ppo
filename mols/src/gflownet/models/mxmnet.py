...
def rdkit_conformation(mol, n=5, addHs=False, timeout_seconds=0):
    if addHs:
        mol = AllChem.AddHs(mol)
    embed_params = params
    if timeout_seconds and hasattr(params, "timeout"):
        embed_params = copy.copy(params)
        embed_params.timeout = int(timeout_seconds)
    confs = AllChem.EmbedMultipleConfs(mol, numConfs=n, params=embed_params)
    minc, aminc = 1000, 0
...
def mol2graph(mol, timeout_seconds=0):
    mol = AllChem.AddHs(mol)
    N = mol.GetNumAtoms()
    try:
        pos = rdkit_conformation(mol, timeout_seconds=timeout_seconds)
