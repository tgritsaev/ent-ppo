...
from gflownet.algo.backward_policy import shifted_model_log_p_b, tlm_loss_from_log_p_b
...
class TrajectoryBalance(GFNAlgorithm):
    ...
    def compute_batch_losses(
        ...
        backward_model: Optional[TrajectoryBalanceModel] = None,
    ):
        ...
        if self.cfg.do_parameterize_p_b:
            ...
            if backward_model is not None:
                with torch.no_grad():
                    log_p_B, _ = shifted_model_log_p_b(backward_model, batch, batched_cond_info)
            elif self.global_cfg.algo.backward_approach == "tlm":
        ...
        if self.cfg.do_sample_p_b and proxy_eubo_mask.any():
            ...
            add_bound_metrics(info, bound_metric, log_rewards, traj_log_p_F - traj_log_p_B, masks)
        if self.ctx.has_n() and self.cfg.do_predict_n:
    ...
    def compute_backward_policy_loss(self, model: TrajectoryBalanceModel, batch: gd.Batch):
        if not self.cfg.do_parameterize_p_b:
            raise ValueError("TLM backward learning requires cfg.algo.tb.do_parameterize_p_b=True.")
        num_trajs = int(batch.traj_lens.shape[0])
        batch_idx = torch.arange(num_trajs, device=batch.x.device).repeat_interleave(batch.traj_lens)
        batched_cond_info = batch.cond_info[batch_idx] if getattr(batch, "cond_info", None) is not None else None
        log_p_B, mask = shifted_model_log_p_b(model, batch, batched_cond_info)
