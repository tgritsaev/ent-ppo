...
class GraphSampler:
    ...
    @staticmethod
    def _graph_complexity(g: Graph) -> int:
        """Monotone-ish size used to catch invalid backward transitions."""
        try:
            node_attrs = sum(len(attrs) for _, attrs in g.nodes(data=True))
            edge_attrs = sum(len(attrs) for *_, attrs in g.edges(data=True))
            return len(g.nodes) + len(g.edges) + node_attrs + edge_attrs
        except Exception:
            return len(g)
    ...
    def sample_from_model(self, model: nn.Module, n: int, cond_info: Optional[Tensor], random_action_prob: float = 0.0):
        ...
        fwd_logprobs: List[List[Tensor]] = [[] for _ in range(n)]
        fwd_full_logprobs: List[List[List[Tensor]]] = [[] for _ in range(n)]
        ...
        for t in range(self.max_len):
            ...
            all_log_probs = fwd_cat.logsoftmax()
            # Step each trajectory, and accumulate statistics
            for i, j in zip(not_done(range(n)), range(n)):
                fwd_logprob[i].append(log_probs[j].unsqueeze(0))
                fwd_logprobs[i].append(log_probs[j].detach().unsqueeze(0))
                fwd_full_logprobs[i].append(
                    [
                        log_prob[fwd_cat.slice[k][j] : fwd_cat.slice[k][j + 1]].detach()
                        for k, log_prob in enumerate(all_log_probs)
                    ]
                )
        ...
        for i in range(n):
            ...
            data[i]["fwd_logprobs"] = torch.stack(fwd_logprobs[i]).reshape(-1)
            data[i]["fwd_full_logprobs"] = fwd_full_logprobs[i]
    ...
    def sample_backward_from_graphs(
        ...
        max_backward_steps = self.max_backward_steps if self.max_backward_steps is not None else self.max_len + 1
        step = 0
        while sum(done) < n:
            if step >= max_backward_steps:
                unfinished = [i for i, is_done in enumerate(done) if not is_done]
                sizes = [self._graph_complexity(graphs[i]) for i in unfinished[:8]]
                message = (
                    "Backward sampling exceeded max_backward_steps="
                    f"{max_backward_steps}; unfinished={unfinished[:8]} complexities={sizes}"
                )
                print(f"[error][backward_sampling] {message}", flush=True)
            ...
            for i, j in zip(not_done(range(n)), range(n)):
                if not done[i]:
                    g = graphs[i]
                    old_complexity = self._graph_complexity(g)
                    b_a = graph_bck_actions[j]
                    gp = self.env.step(g, b_a)
                    f_a = self.env.reverse(g, b_a)
                    graphs[i], f_a = relabel(gp, f_a)
                    new_complexity = self._graph_complexity(graphs[i])
                    if len(graphs[i]) != 0 and new_complexity >= old_complexity:
                        message = (
                            "Backward sampling made no progress: "
                            f"old_complexity={old_complexity} new_complexity={new_complexity} action={b_a}"
                        )
                        print(f"[error][backward_sampling] {message}", flush=True)
                        raise RuntimeError(message)
